...
export async function apiFetchBlob(
  ...
  let res = await fetch(`${config.apiBaseUrl}${path}`, { ...options, headers });

  if (res.status === 401 && token) {
    const newToken = await refreshAccessToken();
    if (newToken) {
      headers.Authorization = `Bearer ${newToken}`;
      res = await fetch(`${config.apiBaseUrl}${path}`, { ...options, headers });
    }
  }
  ...
  if (!res.ok) {
    ...
    const message =
      ...
      res.statusText ??
      `HTTP ${res.status}`;
    const code = raw?.error?.code ?? raw?.code ?? `HTTP_${res.status}`;
    throw { status: res.status, error: { code, message }, raw };
  }
